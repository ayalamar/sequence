...
for (periodno in periods){
  ...
  allpaths <- NA
    ...
    #taskdf$garage_location <- taskdf$prehome # need for preq exp
    ...
    taskpaths <- allpaths %>%
      filter(garagelocation == garageno) %>%
      filter(task == tasknum) %>%
      group_by(npoint) %>%
    ...
    plotfilename <- sprintf('_paths/paths_explicit%d_task%d_%sadaptation.svg', garageno,tasknum,periodno)
