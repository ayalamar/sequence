...
#### remove the headers 
library(dplyr)
subject_numbers <- c(1:9) # edit participant #s
...
for (subject in 1:length(subject_numbers)){
  ...
  for (task in 0:12){
    ...
    infile_name <- sprintf('p00%d-%d.csv', subject, task)
  ...
  if (data$rotation[1] != 0 ) {
    ...
    outfile_name = sprintf('p00%0d-%d.txt', subject, task)
