import collections
import os
from utility import counterbalance, realign, find_diff, speed, writedata, calibrate
...
def runtrial(cfg, block, trial_number):
    ...
    while trial_data['step'] is not 'end':
        ...
        trial_data['cursor_target_diff'] = find_diff(block['target_stim'][trial_number].pos, trial_data['cursor_pos'][-1])
        trial_data['cursor_garage_diff'] = find_diff(block['garage_stim'][trial_number][2].end,
                                                     trial_data['cursor_pos'][-1])
        print(trial_data['step'])
        ...
        if trial_data['step'] is 'leave_home':
            trial_data['num_step'].append(1)
        ...
        if trial_data['step'] is 'reach_target':
            trial_data['num_step'].append(2)

            if trial_data['time'][-1] >= trial_data['reach_target_timer_start'] + 1.5:
                cfg['textstim'].draw()
        ...
        if trial_data['step'] is 'waiting_on_target':
            trial_data['num_step'].append(3)
            if acquire_target(cfg, block, trial_data):
                trial_data['step'] = 'reach_garage'
                trial_data['clamp'] = 'horizontal_clamp'
        ...
        if trial_data['step'] is 'reach_garage':
                ...
                if acquire_garage(cfg, block, trial_data):
                    trial_data['step'] = 'reach_home'
                    cfg['cursorstim'].opacity = 0
                    cfg['home_stim'].opacity = 1

        if trial_data['step'] is 'reach_home':
...
def acquire_target(cfg, block, trial_data):
    if block['block_name'] == 'Reach with cursor':
        return trial_data['cursor_target_diff'] < cfg['acquire_target_limit'] * cfg['height'] and\
               speed(trial_data['cursor_pos'],trial_data['time'], cfg['speed_limit'])
    else:
...
def acquire_garage(cfg, block, trial_data):
    if block['block_name'] == 'Reach with cursor':
        return trial_data['cursor_garage_diff'] < cfg['acquire_garage_limit']
    else:
        return find_diff( trial_data['cursor_astarget_position'], trial_data['cursor_pos'][-1]) >= cfg['far_garage_distance']
