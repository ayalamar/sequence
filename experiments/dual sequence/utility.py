...
def writedata(cfg, block, trial_data):
    ...
    with open(outfilename, 'wb') as csvfile:
        ...
        csv_writer.writerow(['time', 'trial', 'cursorx', 'cursory', 'hand_x', 'hand_Y', 'rotation', 'step', 'targetangle',
                             'targetposx', 'targetposy', 'garagelocation', 'garageposx',
                             'garageposy', 'homex', 'homey','group', 'trial_type'])
        for index, (time, cursor, hand) in enumerate(zip(trial_data['time'], trial_data['cursor_pos'], trial_data['hand_pos'])):
            ...
            if block['block_name'] == 'Reach with cursor':
                ...
                t_output = -1
            csv_writer.writerow([time, trial_number, cursor[0], cursor[1], hand[0], hand[1], block['rotation'][trial_number],
                                trial_data['num_step'][index-1], trial_data['target_angle'], block['target_stim'][trial_number].pos[0],
...
def speed(pos,time, limit):
    current_time = time[-1]
    first_time = next(x[0] for x in enumerate(time) if x[1] >= current_time - 0.01)
    if pos[first_time] == pos[-1]:
        return 1
    else:
        return 0
